...
describe('notification bar', () => {
  ...
  describe('with cookieName', () => {
    ...
    it('should set the cookie when max-age property is provided', () => {
      showNotificationBarIfNeeded({ parentElement: mainContent, message: 'foo', cookieName: COOKIE_ACCEPTED_NAME, maxage: 10000 })
      document.body.querySelector('.accept').click()

      expect(cookie(COOKIE_ACCEPTED_NAME)).to.equal('1')
    })

    it('should delete the cookie when it is expired', () => {
      const start = new Date().getTime()
      let end = start
      showNotificationBarIfNeeded({ parentElement: mainContent, message: 'foo', cookieName: COOKIE_ACCEPTED_NAME, maxage: 1000 })
      document.body.querySelector('.accept').click()
      new Promise(() => {
        setTimeout(() => {
          expect(cookie(COOKIE_ACCEPTED_NAME)).to.equal(undefined)
        }, 1200)
      })
    })
  })
